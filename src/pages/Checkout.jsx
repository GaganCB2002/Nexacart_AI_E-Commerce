...
const Checkout = () => {
    ...
    // Determine items to checkout: either directly selected product or cart items
    ...
    const calculateTotal = () => {
        const sub = checkoutItems.reduce((total, item) => total + (item.price * item.quantity), 0);
        return sub;
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const orderPayload = {
            items: checkoutItems.map(item => item.id), // Simplified for prototype
            totalAmount: total,
            userId: user?.userId, // Send User ID for isolation
            paymentMethod: paymentMethod
        };

        try {
            const response = await api.post('/orders/place', orderPayload);
            const orderId = response.data.transactionId || 'ORD' + Date.now();
            ...
            alert("Order failed. Please check backend connection.");
        }
    };
