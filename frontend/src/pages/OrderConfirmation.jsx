        ...
        const fetchOrder = async () => {
            try {
                ...
                if (response.data.status === 'FOUND') {
                    // Adapt the response to match the UI expectation
                    // The backend returns { status, orderStatus, amount } but we need full details
                    // Currently backend track endpoint is minimal. We need to enhance it or client side mock for now if backend is incomplete?
                    // Wait, the backend OrderController.trackOrder returns minimal info (status, amount). 
                    // It does NOT return items or shipping address. 
                    // This is a missing feature in backend.

                    // For now, to solve the "leak", we must NOT show local storage data of another user.
                    // But if backend doesn't return full data, the UI will break.

                    // Quick fix: Only show if we can verify ownership? 
                    // Better: Update backend to return full order details.

                    // Let's assume for this step I will just use the minimal data or fail gracefully.
                    // Actually, to display the "Order Confirmation" properly with items, I need the items.
                    // The backend `trackOrder` returns `Optional<Order>`, but the mapped response in Controller only has `status` and `amount`.

                    // I need to update OrderController.java to return full details first?
                    // Or I can read from `api.get('/orders/user/{userId}')` and find the order?

                    // Let's use the local storage *only if* it matches the ID and we just placed it? 
                    // No, that's still risky if not cleared.

                    // Correct fix: Update Backend to return full order object.
                }
            } catch (err) {
